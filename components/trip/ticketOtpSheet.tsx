...
function TicketOtpSheet() {
  ...
  const {  allRequests, unAcceptedRequests, currentUpcomingTrip, tripRequestInView, acceptedRequests } = useAppSelector(
  ...
  const startTrip = async (otp: string) => {
    ...
    await FetchService.patchWithBearerToken({
      url: `/user/driver/me/trip/${currentUpcomingTrip?._id}/start-trip`,
      data: {
        requestId: tripRequestInView?._id,
        ticketOtp: otp,
      },
      ...
      .then(async (res) => {
        ...
        if ((code && (code == 200 || code == 201 || code == 400)) && riderRide && currentRide) {
          const rideSaved = acceptedRequests.find(
          ...
          if (!rideSaved) {
            dispatch(
              setTripState({
                ...
                value: [...acceptedRequests, riderRide],
              })
            );
            ...
            dispatch(setTripState({ key: "currentUpcomingTrip", value: currentRide }));
            dispatch(setTripState({key:'tripRequestInView', value: riderRide}));

            dispatch(setTripState({key: 'query', value: RideConstants.query.pause_trip}))
  ...
  const cancelTrip = () => {
    dispatch(setTripState({key:'selectedRoute', value: false}));
    dispatch(setTripState({key:'driverOnline', value: false}));
    dispatch(setTripState({key:'selectedRoute', value: null}));
    dispatch(setTripState({key:'query', value: RideConstants.query.searching}));
    ...
    <PaddedScreen>
      ...
      >
        ...
        >
          <View style={[flex, justifyBetween, { gap: 14 }]}>
            <View>
              <Image
                ...
                source={{
                  uri:
                    (tripRequestInView?.riderPicture as string),
                }}
              />
            </View>
          ...
          <View>
            <Text style={[fw500, fs14, colorBlack]}>
              ₦ {tripRequestInView?.riderCounterOffer}
            </Text>
          </View>
        </View>
        {/* //!Rider Details Block */}
        ...
        <View style={[flex, itemsCenter, gap(20), mXAuto] as ViewStyle[]}>
          <TouchableOpacity
            onPress={() => openWhatsApp(String(tripRequestInView?.riderPhoneNo))}
            style={[
              flex,
              rounded(100),
              gap(10),
          ...
          <TouchableOpacity
            onPress={() =>
              openCallerApp(String(tripRequestInView?.riderPhoneNo))
            }
            style={[
              flex,
              rounded(100),
