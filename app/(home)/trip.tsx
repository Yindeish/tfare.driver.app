...
function Trip() {
  ...
  const { presetTrips, upcomingTrips } = useAppSelector(
  ...
  const getTrips = async () => {
    ...
    await FetchService.getWithBearerToken({
      url: "/user/driver/me/trips/upcoming",
      ...
      .then(async (res) => {
        ...
        const presetTrips = data?.presetTrips;
  ...
  return (
    <SafeScreen>
      <ScrollView
        refreshControl={
          <RefreshControl onRefresh={getTrips} refreshing={loading} />
        }
      >
        <View style={[flexCol, gap(32), p(20), pb(150)]}>
          {/* //!Page Title */}
          <PageTitle title="Trips" backBtnColor={colors.grey600} />
          {/* //!Page Title */}
          ...
          <View style={[wFull, flex, itemsCenter, justifyBetween]}>
            <Text style={[fw700, fs14, neurialGrotesk, colorBlack]}>
              Upcoming Trips
            </Text>

            <TouchableOpacity>
              <Image
                style={[image.w(18), image.h(16)]}
                source={tripImgs.settings}
              />
            </TouchableOpacity>
          </View>
          {/* //!Upcoming Trips Header */}
