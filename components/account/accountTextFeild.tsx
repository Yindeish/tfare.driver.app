...
function AccountTextField({ label, input }: { label: ILabel, input: IInput }) {
    ...
    return (
        <View style={[wFull, flexCol, gap(8)]}>
            <Text style={[c(Colors.light.darkGrey), fs12, fw400]}>{label.text}</Text>
            ...
            />) :
                (<Text style={[py(16), px(10), rounded(10), bg(colors.transparent), c(Colors.light.darkGrey), fs14, fw500, { borderWidth: 0.7, borderColor: Colors.light.border }]}>{input.value}</Text>)
            }
        </View>
    )
}
